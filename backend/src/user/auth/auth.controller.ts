...
export class AuthController {
	...
	@UseGuards(verifyUser)
	@Post('2fa/verify')
	async verify2fa(@Req() req, @Res({ passthrough: true }) res: Response, @Body('code') code: string) {
		const clientID = await this.authService.clientID(req);
	...
	@UseGuards(verifyUser)
	@Post('2fa/login')
	async login2fa(@Req() req, @Res({ passthrough: true }) res: Response, @Body('code') code: string) {
		const clientID = await this.authService.clientID(req);
		const verified = await this.authService.twoFactorAuthVerify(code, clientID);
		if (verified) {
	...
	async sendGameInvite(@Req() req: Request) {
		if (await this.userService.findPrivateGame())
			return await this.userService.sendGameInvite(await this.authService.clientID(req));
		else
			throw new UnauthorizedException();
	}
	...
	async acceptGameInvite(@Req() req: Request) {
		return await this.userService.acceptGameInvite(await this.authService.clientID(req));
	}
